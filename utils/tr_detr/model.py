...
from mchd.encoder import TransformerEncoder
...
class TRDETR(nn.Module):
    ...
    def __init__(self,transformer,transformer_vt,transformer_ot, position_vid_embed, position_obj_embed,txt_position_embed, txt_dim, vid_dim,vid_obj_dim,
        ...
        hidden_dim = transformer_vt.d_model
        ...
        self.input_obj_vid_proj = nn.Sequential(*[
            LinearLayer(vid_obj_dim, hidden_dim, layer_norm=True,dropout=input_dropout, relu=relu_args[0]),
            LinearLayer(hidden_dim, hidden_dim, layer_norm=True,dropout=input_dropout, relu=relu_args[1]),
            LinearLayer(hidden_dim, hidden_dim, layer_norm=True,dropout=input_dropout, relu=relu_args[2])
        ][:n_input_proj])
        ...
        self.saliency_proj1 = nn.Linear(transformer_vt.d_model, transformer_vt.d_model)
        self.saliency_proj2 = nn.Linear(transformer_ot.d_model, transformer_ot.d_model)
        self.trans_memory_proj = nn.Linear(in_features=512,out_features=256)

        self.fuser_vt = VSLFuser(transformer_vt.d_model)
        self.fuser_ot = VSLFuser(transformer_ot.d_model)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_obj_vid, src_obj_vid_mask, ):
        ...
        src_vid = self.encoder(src_vid)

        src_txt_ed = src_txt
        src_vid_ed = src_vid
        src_obj_vid_ed = src_obj_vid
        # VTC
        ...
        src_obj_vid = self.fuser_ot(src_obj_vid, src_txt, src_obj_vid_mask,src_txt_mask)
        # src_obj_vid = torch.randn(32,75,256).to("cuda")



        src_vt = torch.cat([src_vid, src_txt], dim=1)  # (bsz, L_vid+L_txt, d)
        src_ot = torch.cat([src_obj_vid, src_txt], dim=1)
        mask_vt = torch.cat([src_vid_mask, src_txt_mask], dim=1).bool()  # (bsz, L_vid+L_txt)
        mask_ot = torch.cat([src_obj_vid_mask, src_txt_mask], dim=1).bool()
        ...
        pos_ot = torch.cat([pos_obj_vid, pos_txt], dim=1)
        ...
        # )
        # hs = torch.cat([hs, HS],dim=3)
        # hs = self.linear_hs(hs)
        ...
        saliency_scores_refined , memory_after_mr2hd = self.MR2HD(out, memory_fuser, video_length, src_vid_ed)
