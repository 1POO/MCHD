...
class StartEndDataset_subtitles(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.s_feat_dir is not None:
            ...
            if ctx_sl < ctx_vl & ctx_vl >= ctx_ol:
                ctx_vl = ctx_sl
                model_inputs["video_feat"] = model_inputs["video_feat"][:ctx_vl]
                model_inputs["subtitles_feat"] = model_inputs["subtitles_feat"][:ctx_vl]
            if ctx_sl < ctx_ol & ctx_ol >= ctx_vl:
                ctx_ol = ctx_sl
                model_inputs["object_feat"] = model_inputs["object_feat"][:ctx_ol]
                model_inputs["subtitles_feat"] = model_inputs["subtitles_feat"][:ctx_ol]
        ...
        if self.use_tef:
            ...
            tef_sst = torch.arange(0, ctx_sl, 1.0) / ctx_sl
            tef_ved = tef_vst + 1.0 / ctx_vl
            tef_oed = tef_ost + 1.0 / ctx_ol
            tef_sed = tef_sst + 1.0 / ctx_sl
