...
from mchd.gate_fuser import TemporalGatedFusion
from mchd.paint_feat import paint_feat
from mchd.span_utils import generalized_temporal_iou, span_cxw_to_xx
from mchd.BidirectionalLSTM import BLSTM
from modules.transformer import TransformerEncoder
from mchd.CTF import CrossTransformer
# from mchd.MambaDecoder import MambaDecoder
# from mchd.test.memba_fuser import MultiModalEnhancer
...
#     def __init__(self, input_dim=256, hidden_size=256):
#         super().__init__()
#         # 方法一：拼接后输入 BiLSTM
#         self.bilstm = nn.LSTM(
#             input_size=2*input_dim,  # 拼接后输入维度为512
#             hidden_size=hidden_size,
#             bidirectional=True,
#             batch_first=True
#         )
#         # 分类头
#         self.fc = nn.Linear(2*hidden_size, 256)  # 假设输出10类
#
#     def forward(self, x1, x2):
#         # 拼接特征
#         merged = torch.cat([x1, x2], dim=-1)  # (32,75,512)
#         # BiLSTM
#         output, _ = self.bilstm(merged)       # (32,75,512)
#         # 分类
#         output = self.fc(output)           # (32,10)
#         return output
...
class TRDETR(nn.Module):
    ...
    def __init__(self, transformer_v, transformer_o, transformer_t, transformer, position_vid_embed, position_obj_embed,
                 txt_position_embed, txt_dim, vid_dim, vid_obj_dim,
        ...
        # self.transformer_vt = transformer_vt
        # self.transformer_ot = transformer_ot
        self.transformer = transformer
        self.fuser_vot = TransformerEncoder(embed_dim=256,num_heads=4,layers=2).to("cuda")
        ...
        self.input_vid_sub_proj = nn.Sequential(*[
                                                     LinearLayer(vid_dim+sub_dim, hidden_dim, layer_norm=True,
                                                                 dropout=input_dropout, relu=relu_args[0]),
        ...
        self.saliency_proj2 = nn.Linear(transformer_o.d_model, transformer_o.d_model)
        self.trans_memory_proj = nn.Linear(in_features=512, out_features=256)

        self.fuser_vt = VSLFuser(transformer.d_model)  # 3
        self.fuser_ot = VSLFuser(transformer.d_model)  # 1
        self.fuser_vo = VSLFuser(transformer.d_model)  # 2

        # self.mambaDecoder = MambaDecoder()
    ...
    # def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_obj_vid, src_obj_vid_mask, ):
    #
    #     # if src_sub is not None:
    #     #     src_vid = torch.cat([src_vid, src_sub], dim=2) #3332
    #
    ...
    #     src_obj_vid = self.input_obj_vid_proj(src_obj_vid) # 32 75 256
    #
    #     src_vid_copy = src_vid.clone()
    #     src_txt_copy = src_txt.clone()
    #
    ...
    #     pos_obj_vid = self.position_obj_vid_embed(src_obj_vid, src_obj_vid_mask)
    #     # pos_obj_vid = torch.randn(32,75,256).to("cuda")
    #     pos_txt = self.txt_position_embed(src_txt) if self.use_txt_pos else torch.zeros_like(src_txt)  # (bsz, L_txt, d)
    #     video_length = src_vid.shape[1]
    #     object_length = src_obj_vid.shape[1]
    #     txt_length = src_txt.shape[1]
    ...
    #         only_encoder=True
    #     ).transpose(0,1)
    #     src_obj = self.transformer_v(
    #         src=src_obj_vid,
    #         query_embed=None,
    #         mask=~src_obj_vid_mask.bool(),
    #         saliency_proj1=self.saliency_proj1,
    #         pos_embed=pos_obj_vid,
    #         video_length=object_length,
    #         only_encoder=True
    #     ).transpose(0,1)
    #     src_txt = self.transformer_v(
    #         src=src_txt,
    #         query_embed=None,
    #         mask=~src_txt_mask.bool(),
    #         saliency_proj1=self.saliency_proj1,
    #         pos_embed=pos_txt,
    #         video_length=txt_length,
    #         only_encoder=True
    #     ).transpose(0,1)
    #     # CTC
    #     src_txt_ed = src_txt
    ...
    #     src_vid_cls_ed = src_vid.mean(1)  # 1 256
    #     src_txt_cls_ed = src_txt.mean(1)  # 1 256
    #     src_obj_vid_cls_ed = src_obj.mean(1)
    #
    #     src_vid = self.fuser_vt(src_vid, src_obj, src_vid_mask, src_obj_vid_mask)
    #     src_ot = self.fuser_ot(src_obj, src_txt, src_obj_vid_mask, src_txt_mask)
    #     # src_obj_vid = torch.randn(32,75,256).to("cuda")
    #
    #
    #     video_length = src_vid.shape[1]
    #
    #     src = torch.cat([src_vid,src_txt],dim=1)
    #     mask_src = torch.cat([src_vid_mask,src_txt_mask],dim=1).bool()
    #     pos_src = torch.cat([pos_vid,pos_txt],dim=1)
    #
    #     hs, reference,memory_local,saliency_scores = self.transformer(
    #         src, ~mask_src,
    ...
    #     #     video_length=video_length,  # 视频长度
    #     # ) # 32,75,256
    #     # # memory_vt = torch.randn(32,75,256).to("cuda")
    #     # # memory_ot = torch.randn(32,75,256).to("cuda")
    #     # memory_ot = self.transformer_ot(
    #     #     src_ot,
    #     #     ~mask_ot,
    #     #     self.query_embed.weight,
    #     #     pos_ot,
    ...
    #     # memory_ot = torch.randn(32,75,256).to("cuda")
    #     # 到memory融合了
    #     # memory_fuser = torch.cat([memory_vt, memory_ot], dim=2)
    #     # hs, reference = self.blstm(memory_fuser)
    #     # hs,reference = self.mambaDecoder(memory_fuser)
    #     #
    #     # hs = torch.randn(2,32,10,256).to("cuda")
    #     # reference = torch.randn(2,32,10,2).to("cuda")
    #     # 进行分类预测
    #     outputs_class = self.class_embed(hs)  # 进行分类预测 linear层 # (#layers, batch_size, #queries, #classes)
    #     # outputs_class = torch.randn(2,32,10,2).to("cuda")
    #
    #     # 计算边界框预测
    #     reference_before_sigmoid = inverse_sigmoid(reference)  # 反向sigmoid处理
    #     tmp = self.span_embed(hs)  # 计算跨度嵌入 MLP层 获得预测时刻
    ...
    #
    #     # *Refine the highlight score prediction process using the retrieved moments from moment retrieval
    #     saliency_scores_refined , memory_after_mr2hd = self.MR2HD(out, memory_local, video_length, src_vid_ed)
    #
    #     # txt_mem = memory[:, src_vid.shape[1]:] # (1, 0, 256)
    #     #
    #     # vid_mem = memory[:, :src_vid.shape[1]] # (1, 75, 256)
    ...
    #
    #     video_length = src_vid.shape[1]
    #     txt_length = src_vid.shape[1]
    #     object_length = src_vid.shape[1]
    #
    #     src_vid_mask_bool = src_vid_mask.bool()
    #     src_obj_vid_mask_bool = src_obj_vid_mask.bool()
    #     src_txt_mask_bool = src_txt_mask.bool()
    #
    #     src_vid = self.transformer_v(
    #         src=src_vid,
    #         query_embed=None,
    #         mask=~src_vid_mask_bool,
    #         saliency_proj1=self.saliency_proj1,
    #         pos_embed=pos_vid,
    #         video_length=video_length,
    #         only_encoder=True
    ...
    #
    #     # CTC
    #     src_txt = src_txt.transpose(0, 1)
    #     src_vid = src_vid.transpose(0, 1)
    #     src_obj = src_obj.transpose(0, 1)
    #
    #     src_txt_ed = src_txt
    #     src_vid_ed = src_vid
    #     src_obj_vid_ed = src_obj
    #     # VTC
    #     src_vid_cls_ed = src_vid.mean(1)  # 1 256
    #     src_txt_cls_ed = src_txt.mean(1)  # 1 256
    #     src_obj_vid_cls_ed = src_obj_vid.mean(1)
    #
    #     src_ov = self.fuser_vt(src_vid, src_obj, src_vid_mask, src_obj_vid_mask)
    #     src_ot = self.fuser_ot(src_obj, src_txt, src_obj_vid_mask, src_txt_mask)
    #
    #     memory_local = torch.cat((src_ov,src_ot),dim=2)
    #     # hs, reference = self.blstm(memory_local)
    #     hs,reference = self.mambaDecoder(memory_local)
    #     #
    #     # hs = torch.randn(2,32,10,256).to("cuda")
    #     # reference = torch.randn(2,32,10,2).to("cuda")
    #     # 进行分类预测
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_obj, src_obj_mask, src_sub, src_sub_mask):
        ...
        if src_sub is not None:
            src_vid = torch.cat([src_vid, src_sub], dim=2) #3330
        # src_sub = self.input_sub_proj(src_sub)
        src_vid = self.input_vid_sub_proj(src_vid)  # 32 75 256
        src_txt = self.input_txt_proj(src_txt)  # 32 26 256
        ...
        pos_vid = self.position_vid_embed(src_vid, src_vid_mask)  # (bsz, L_vid, d)
        # pos_vid = torch.randn(32,75,256).to("cuda")
        pos_obj_vid = self.position_obj_vid_embed(src_obj, src_obj_mask)
        # pos_obj_vid = torch.randn(32,75,256).to("cuda")
        pos_txt = self.txt_position_embed(src_txt) if self.use_txt_pos else torch.zeros_like(src_txt)  # (bsz, L_txt, d)
        video_length = src_vid.shape[1]
        object_length = src_obj.shape[1]
        txt_length = src_txt.shape[1]
        # CTC
        src_txt_ed = src_txt
        src_vid_ed = src_vid
        ...
        # src_vid = self.fuser_vo(src_vid, src_obj, src_vid_mask, src_obj_mask)
        # src_vid = self.fuser_vt(src_vid, src_txt, src_vid_mask, src_txt_mask)
        src_vid_trans = src_vid.transpose(1,0)
        src_obj_trans = src_obj.transpose(1,0)
        src_txt_trans = F.pad(src_txt, (0, 0, 0, (75-src_txt.shape[1]))).transpose(1,0)
        src_vid = self.fuser_vot(src_vid_trans,src_obj_trans,src_txt_trans).transpose(1,0)

        # new fuser
        # src_vid = self.fuser(src_vid,src_obj,src_txt,src_txt_mask)
        # old fuser
        # src_vid = self.fuser_vt(src_vid,src_txt,src_vid_mask,src_txt_mask)

        # src_vid = self.fuser_vt(src_vid,src_txt,src_vid_mask,src_txt_mask)
        # src_vid = self.fuser_vt(src_vid, src_obj, src_vid_mask, src_obj_mask) # 32 75 256 增强的视频特征
        # src_obj = self.fuser_ot(src_obj, src_txt, src_obj_mask, src_txt_mask) # 增强的物体特征
        # src_obj_vid = torch.randn(32,75,256).to("cuda")

        video_length = src_vid.shape[1]

        # 增强特征融合
        # src_vid = torch.cat([src_vid, src_obj],dim=2)
        # src_vid = self.linear(src_vid)

        # src_vid = self.gate(src_vid, src_sub)
        # src_vid = self.blstm(src_vid,src_sub)
        src = torch.cat([src_vid, src_txt], dim=1)
        src_ot = torch.cat([src_obj, src_txt], dim=1)
        mask_src = torch.cat([src_vid_mask, src_txt_mask], dim=1).bool()
        mask_ot = torch.cat([src_obj_mask, src_txt_mask], dim=1).bool()
        pos_src = torch.cat([pos_vid, pos_txt], dim=1)
        ...
        #     video_length=video_length,  # 视频长度
        #     only_encoder = True
        # ).transpose(1,0) # 32,75,256
        # # memory_vt = torch.randn(32,75,256).to("cuda")
        # # memory_ot = torch.randn(32,75,256).to("cuda")
        # memory_ot = self.transformer_o(
        #     src_ot,
        #     ~mask_ot,
        #     self.query_embed.weight,
        #     pos_src,
        ...
        # 到memory融合了
        # memory_fuser = torch.cat([memory_vt, memory_ot], dim=2)
        # memory_fuser = self.trans_memory_proj(memory_fuser)
        # hs, reference = self.blstm(memory_fuser)
        # hs,reference = self.mambaDecoder(memory_fuser)
        #
        # hs = torch.randn(2,32,10,256).to("cuda")
    ...
    def MR2HD(self, out, memory, video_length, src_vid_ed):
        ...
        """
        # memory = self.trans_memory_proj(memory)
        ...
        cosine_similarity_matrix = torch.matmul(sliced_features_global_expanded, src_vid_ed).squeeze(
            1)  # shape: (32, 1, 75)
        #
...
def build_model(args):
    ...
    # transformer_vt = build_transformer(args)
    # transformer_ot = build_transformer(args)
    transformer_v = build_transformer(args).to(device)
    transformer_o = build_transformer(args).to(device)
    transformer_t = build_transformer(args).to(device)
    ...
    if args.s_feat_dir is None:
        model = TRDETR(
            # transformer_vt,
            # transformer_ot,
            transformer_v, transformer_o, transformer_t,
            transformer,
