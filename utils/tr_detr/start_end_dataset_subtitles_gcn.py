...
class StartEndDataset_subtitles_gcn(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,q_feat_dirs, s_feat_dirs=None,
        ...
        if self.use_tef:
            ...
            if self.use_sub_tef:
                model_inputs["subtitles_feat"] = torch.cat(
                    [model_inputs["subtitles_feat"], tef], dim=1)  # (Lv, Dv+2)
    ...
    def _get_subtitles_feat_by_vid(self, vid):
        s_feat_list = []
        for s_feat_path in self.s_feat_dirs:
            s_feat_path = join(s_feat_path, f"{vid}.npz")
        ...
        s_feat = np.concatenate(s_feat_list, axis=1)

        return torch.from_numpy(s_feat)  # (D, ) or (Lq, D)


def start_end_collate_subtitles_gcn(batch):
...
def prepare_batch_inputs_subtitles_gcn(batched_model_inputs, device, non_blocking=False):
    model_inputs = dict(
