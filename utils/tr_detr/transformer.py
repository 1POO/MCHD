...
from .paint_feat import paint_feat
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # 前向传播：两层卷积，每层后加批归一化与 ReLU 激活，并加上残差连接
...
class Blocks(nn.Module):
    ...
    def __init__(self, d_model, n_layers):
        super().__init__()
        # 根据 n_layers 数量堆叠 Block 层
        blocks = [Block(d_model) for _ in range(n_layers)]
        # 使用 ModuleList 存储所有的 Block 层
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.blocks):
            out = layer(out)  # 每层输入都会经过对应的 Block 层处理
        return out  # 返回最终输出
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,# num_encoder_layers = 6
        ...
        self.resnet = Blocks(d_model,5)


        
        # self.saliency_proj2 = nn.Linear(d_model, d_model) global

        self.highlight_ref = nn.Parameter(torch.randn(d_model))
        self.nhead = nhead
    ...
    def forward(self, src, mask, query_embed, pos_embed, saliency_proj1, video_length=None,only_encoder=False): # 和主函数权重共享
        """
        Args:
            src: (batch_size, L, d)
            mask: (batch_size, L)
        ...
        if only_encoder:
            ...
            # query_embed: (#queries, d) -> (queries, bs, d)
            ...
            # pos_embed_local = pos_embed[:, :]
            #
            # memory_local = self.encoder2(memory_local, src_key_padding_mask=mask_local, pos=pos_embed_local)

            # *Use highlight scores to suppress feature expressions in non-highlight clips
            # memory_local = self.HD2MR(memory_local, saliency_proj1, src, video_length, mask_local, pos_embed_local)
            return memory
        else:
            # # flatten NxCxHxW to HWxNxC
            l, bs, d = src.shape
            ...
            memory = src.permute(1, 2, 0)
            memory = self.resnet(memory)
            memory = memory.permute(2, 0, 1)

            memory_local = memory
            mask_local = mask[:, :]
            pos_embed_local = pos_embed[:, :].permute(1,0,2)

            # memory_local = self.encoder2(memory_local, src_key_padding_mask=mask_local, pos=pos_embed_local)

            # *Use highlight scores to suppress feature expressions in non-highlight clips
            # memory_local, saliency_scores = self.HD2MR(memory_local, saliency_proj1, src, video_length, mask_local,
            #                                            pos_embed_local)

            tgt = torch.zeros(refpoint_embed.shape[0], bs, d).cuda()  # [10,32,256]
            # 将增强的联合特征输入到Transformer解码器和预测头中，得到最终的检索时刻。
            # paint_feat(memory_local)
            # for _ in range(3):
            ...
            # trans_memory = self.linear(trans_memory)
            # memory_local = trans_memory.transpose(2, 0)
            # memory_local = memory_local.transpose(0, 1)  # (batch_size, L, d)
            saliency_scores = (torch.sum(saliency_proj1(memory_local), dim=-1) / np.sqrt(self.d_model))
    ...
    def HD2MR(self, memory_local, saliency_proj1, src, video_length, mask_local, pos_embed_local):
        ...
        2. 用余弦相似度权重增强特征
        """
        # 维度转换 [seq_len, batch, d_model] -> [batch, seq_len, d_model]
        vid_mem = memory_local.transpose(0, 1)
        batch_size, seq_len, d_model = vid_mem.shape

        # 核心修改点 ----------------------------------------------------
        # 生成可学习的高光参考向量（参数化）
        highlight_ref = self.highlight_ref # [d_model]

        # 计算余弦相似度（逐片段与参考向量比较）
        vid_mem_norm = F.normalize(vid_mem, p=2, dim=-1)  # [batch, seq_len, d_model]
        ref_norm = F.normalize(highlight_ref, p=2, dim=-1)  # [d_model]
        saliency_scores = torch.matmul(vid_mem_norm, ref_norm)  # [batch, seq_len]

        # 归一化为权重
        weight = torch.softmax(saliency_scores, dim=-1)  # [batch, seq_len]
        # ------------------------------------------------------------

        # 特征增强（与原方法相同）
        src_vid = src[:video_length]  # [seq_len, batch, d_model]
        src_vid_wg = src_vid * weight.permute(1, 0).unsqueeze(-1)  # 维度对齐 [seq_len, batch, 1]

        # 残差连接 + 编码
        memory_local = memory_local + src_vid_wg
        memory_local = self.encoder(memory_local,
                                    src_key_padding_mask=mask_local,
                                    pos=pos_embed_local)
        return memory_local

    # def HD2MR(self, memory_local, saliency_proj1, src, video_length, mask_local, pos_embed_local):
    #     """
    #     HD2MR模块的主要目标是通过Highlight Detection任务生成的highlight scores（高光分数）来增强Moment Retrieval任务的性能
    #     """
    #
    #     vid_mem = memory_local.transpose(0, 1)
    #
    #     # saliency scores 通过多头注意力机制（Multi-Head Attention, MHA）和线性层，从联合特征  Z 中生成每个视频片段的highlight scores
    ...
    #
    #     # 为了过滤掉非高光信息，将highlight scores  H 与联合特征 Z 相乘，得到增强的联合特征 src_vid_wg
    #     src_vid_wg = src_vid * weight.transpose(1, 0).unsqueeze(-1)
    #
    #     # 将增强的联合特征和原始联合特征相加再次输入到多头注意力机制中进行编码，得到感知高光分数的联合特
    #     memory_local = memory_local + src_vid_wg
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        if not self.rm_self_attn_decoder:
            ...
            tgt2 = self.self_attn(q, q-k, value=v, attn_mask=tgt_mask,
