...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
    ...
    # 假设 ap_array 是由 `submission` 和 `ground_truth` 计算得到的数组
    ap_array = np.array(list(qid2ap_list.values()))  # #queries x #thresholds

    # 计算平均值时，忽略 NaN 值
    ap_thds = np.nanmean(ap_array, axis=0)  # 在维度0上求平均并忽略NaN

    # 如果 ap_thds 变为标量，可以通过以下方式确保它是数组
    if isinstance(ap_thds, np.float64):
        ap_thds = np.array([ap_thds])

    # 确保 ap_thds 和 iou_thds 都是迭代对象
    iou_thd2ap = dict(zip([str(e) for e in iou_thds], ap_thds.tolist()))  # 这里把 ap_thds 转换成列表
...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.3, 0.7, 3)):
...
def eval_moment_retrieval(dest_name,submission, ground_truth, verbose=True):
    if "charades" in dest_name:
    ...
    if num_workers > 1:
        with mp.Pool(num_workers) as pool:
            for idx, w_idx, score in pool.imap_unordered(
                ...
                ap_scores[idx, w_idx] = float(score)
    else:
...
def eval_submission(dest_name,submission, ground_truth, verbose=True, match_number=True):
    """
    Args:
    ...
    if "pred_relevant_windows" in submission[0]:
        moment_ret_scores = eval_moment_retrieval(
            dest_name=dest_name,submission=submission, ground_truth=ground_truth, verbose=verbose)
        eval_metrics.update(moment_ret_scores)
        if "charadesSTA" in dest_name:
