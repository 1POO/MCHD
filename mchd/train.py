...
from start_end_dataset_subtitles import StartEndDataset_subtitles, prepare_batch_inputs_subtitles, \
    start_end_collate_subtitles
...
from loss_fun.new_t2v import GlobalTextFrameAlign
import logging
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        if opt.s_feat_dir is None:
            ...
            model_inputs, targets = prepare_batch_inputs_subtitles(opt,batch[1], opt.device, non_blocking=opt.pin_memory)
        ...
        src_txt_mask, src_vid_mask, src_sub_mask, src_obj_mask = (
            model_inputs['src_txt_mask'],
            model_inputs['src_vid_mask'],
            model_inputs['src_sub_mask'],
            model_inputs['src_obj_mask'])

        pos_mask_v = targets['src_pos_mask_v']
        pos_mask_o = targets['src_pos_mask_o']
        pos_mask_s = targets['src_pos_mask_s']
        ...
        new_loss = GlobalTextFrameAlign()
        loss_txt_vid_aligh = new_loss(vid_feat=src_vid_ed, txt_feat=src_txt_ed, pos_mask=pos_mask_v,
                                       vid_mask=src_vid_mask.bool(), txt_mask=src_txt_mask.bool())
        ...
        losses = (losses
                      ...
                      + opt.new_loss_coef * loss_txt_vid_aligh
        ...
        loss_meters["loss_ktal"].update(loss_txt_vid_aligh)
    ...
    tb_writer.add_scalar("Train/ktal", loss_txt_vid_aligh, epoch_i + 1)
    tb_writer.add_scalar("Train/local", loss_vid_txt_align, epoch_i + 1)
