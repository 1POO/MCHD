...
from mchd.interaction.triCross import UniformTriCrossAttention

# from mchd.fuser import fuser_loop
...
class TRDETR(nn.Module):
    ...
    def __init__(self, transformer, position_vid_embed, position_obj_embed, txt_position_embed, txt_dim, vid_dim,
        ...
        # self.fuser_vo = VSLFuser(transformer.d_model)
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_obj, src_obj_mask, src_sub, src_sub_mask):
        ...
        src_sub = self.input_sub_proj(src_sub)  # 32 26 256
        ...
        # src_vid = self.mamba1(src_vid)
        # src_txt = self.mamba2(src_txt)
        # src_obj = self.mamba3(src_obj)

        src_obj = self.fuser_ot(src_obj, src_txt, src_obj_mask, src_txt_mask)
        src_sub = self.fuser_st(src_sub, src_txt, src_sub_mask, src_txt_mask)
        ...
        src_vid = self.cross(src_vid, src_obj, src_sub)
        # src_vid = torch.cat((src_vid, src_obj, src_sub),dim=2)
        # src_vid = self.input_vos(src_vid)
        src_vid_re = src_vid+src_vid_ed

        src_vid = self.fuser_vt2(src_vid_re, src_txt, src_vid_mask, src_txt_mask)

        src_vid = self.gate(src_vid, src_vid_re)

        pos_vid = self.position_vid_embed(src_vid, src_vid_mask)  # (bsz, L_vid, d)

        video_length = src_vid.shape[1]

        hs, reference, saliency_scores = self.transformer_v(
            src_vid,  # 输入特征
