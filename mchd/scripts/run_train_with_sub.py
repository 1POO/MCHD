...
if "clip" in t_feat_type:
    t_feat_dirs.append(f"{feat_root}/clip_text_features")
    t_feat_dim += 512
else:
...
# 字幕特征
if s_feat_type == "clip":
    s_feat_dir = os.path.join(f"{feat_root}/clip_features_s")
    s_feat_dim = 512
else:
...
# 设置 PYTHONPATH
os.environ["PYTHONPATH"] = f".../MCHD/:{os.environ.get('PYTHONPATH', '')}"
...
if len(sys.argv) > 1:
    command.extend(sys.argv[1:],env=os.environ.copy())
