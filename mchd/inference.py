...
from mchd.start_end_dataset_subtitles import start_end_collate_subtitles, prepare_batch_inputs_subtitles, \
    StartEndDataset_subtitles
...
from mchd.config import TestOptions
from mchd.model import build_model
from mchd.span_utils import span_cxw_to_xx
from mchd.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from mchd.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from mchd.postprocessing_mchd import PostProcessorDETR
...
def eval_epoch_post_processing(submission, opt, gt_data, save_submission_filename):
    ...
    if opt.eval_split_name in ["val"]:  # since test_public has no GT
        metrics = eval_submission(
            dest_name=opt.dset_name,submission=submission, ground_truth=gt_data,
    ...
    if opt.nms_thd != -1:
        ...
        if opt.eval_split_name == "val":
            metrics_nms = eval_submission(
                dest_name=opt.dset_name,submission=submission_after_nms, ground_truth=gt_data,
                verbose=opt.debug, match_number=not opt.debug
            )
...
def compute_hl_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        if opt.s_feat_dir is None:
            ...
            model_inputs, targets = prepare_batch_inputs_subtitles(opt, batch[1], opt.device,
...
def setup_model(opt):
    ...
    # 创建参数字典,只包含需要梯度的参数
    # 创建一个包含所有需要梯度的模型参数的字典列表:
    # 1. model.named_parameters() 返回模型中所有参数的(名称,参数)元组
    # 2. if p.requires_grad 筛选出需要计算梯度的参数
    # 3. 将筛选出的参数放入字典中,key为"params"
    param_dicts = [{"params": [p for n, p in model.named_parameters() if p.requires_grad]}]
    # 创建AdamW优化器,设置学习率和权重衰减
    optimizer = torch.optim.AdamW(param_dicts, lr=opt.lr, weight_decay=opt.wd)
    # 创建学习率调度器,在opt.lr_drop步后降低学习率
    # 创建学习率调度器,使用StepLR策略
    # 每经过opt.lr_drop 400步,学习率会按默认的gamma=0.1进行衰减
...
def start_inference(train_opt=None, split=None, splitfile=None):
    ...
    if opt.s_feat_dir is None:
        ...
        print("Video+gcn_Sub Evaluation")
        eval_dataset = StartEndDataset_subtitles_gcn(
            dset_name=opt.dset_name,
            data_path=opt.eval_path,
            v_feat_dirs=opt.v_feat_dirs,
            q_feat_dirs=opt.t_feat_dirs,
        ...
        print("Video+Sub Evaluation")
        eval_dataset = StartEndDataset_subtitles(
            dset_name=opt.dset_name,
            data_path=opt.eval_path,
            v_feat_dirs=opt.v_feat_dirs,
            q_feat_dirs=opt.t_feat_dirs,
    ...
    save_submission_filename = "{}_{}_submission.jsonl".format(
