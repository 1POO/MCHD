...
class T2V(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.video_length = 75  # 固定视频长度（根据实际调整）

    def forward(self, src, src_key_padding_mask=None, pos=None):
        """
        Input:
            src: (bs, T+S, d_model) → T=75(video), S=查询长度
            src_key_padding_mask: (bs, T+S) → 视频部分为0表示有效，1表示padding
            pos: (T+S,) → 位置编码（可选）
        Output:
            enhanced_src: (bs, T+S, d_model) → 增强后的多模态特征
        """
        bs, total_len, _ = src.shape
        # 分割输入：视频部分 + 查询部分
        video_src = src[:, :self.video_length, :]
        query_src = src[:, self.video_length:, :]

        # 生成位置编码（可选）
        if pos is not None:
            pos = pos.unsqueeze(0).unsqueeze(1)  # (1, total_len, d_model)
            video_src = video_src + pos[:, :self.video_length, :]
            query_src = query_src + pos[:, self.video_length:, :]

        # 构建掩码：视频部分只能关注自身，查询可以关注所有位置
        # video_mask: (1, T, T) → 允许视频关注自己
        video_mask = torch.triu(torch.ones(self.video_length, self.video_length, dtype=torch.bool), diagonal=1)
        # query_mask: (1, S, total_len) → 查询可以关注视频+S中的所有位置（根据需求调整）
        query_mask = torch.ones(1, query_src.shape[1], total_len, dtype=torch.bool)

        # 合并输入并应用编码器层
        combined_src = torch.cat([video_src, query_src], dim=1)
        encoder_output = self.encoder_layer(
            combined_src,
            attn_mask=torch.cat([video_mask, query_mask], dim=1),  # (num_heads, S+T, T+S)
            key_padding_mask=src_key_padding_mask.unsqueeze(1),  # (bs, 1, total_len)
        )
...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, saliency_proj1, video_length=None,
                only_encoder=False):  # 和主函数权重共享
        """
        Args:
            src: (batch_size, L, d)
            mask: (batch_size, L)
        ...
        if only_encoder:
            ...
            memory=self.stacked_lstm(memory)
            memory_local = memory
            mask_local = mask[:, :]
            pos_embed_local = pos_embed[:, :]
            ...
            saliency_scores = (torch.sum(saliency_proj1(memory_local), dim=-1) / np.sqrt(self.d_model)).transpose(0, 1)
            ...
            return hs, references, saliency_scores
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def forward_post(self,
        ...
        # print('before src shape :', src.shape)
        pos_src = self.with_pos_embed(src, pos)
        global_token, q, k, v = src[0].unsqueeze(0), pos_src[1:video_length + 1], pos_src[video_length + 1:], src[
                                                                                                              video_length + 1:]
